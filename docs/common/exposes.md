...
Cпособности устройств это абстрактные сущности, на которых основана работа [веб-интерфейса](/web/), работа сервиса [HOMEd Cloud](/cloud/), а также работа функции [Home Assistant MQTT Discovery](https://www.home-assistant.io/integrations/mqtt/#mqtt-discovery). Например, способность `temperature` указывает на наличие датчика темературы, а `light` описывает осветительный прибор.
