...
В качестве `device` может выступать как идентификатор, так и имя устройства, даже если оно содержит проблелы или спецсимволы. Примеры:

```
zigbee/00:12:4b:00:22:14:65:02
```

```
zigbee/waterCounter/3
```
...
Вместо указания конкретного времени можно использовать ключевые слова `sunrise` и `sunset`, которые будут инерпретироваться как время рассвета и заката соответственно, а также можно корректировать эти значения, добавив и или отняв необходимое число минут, например `sunrise + 20` (через 20 минут после рассвета).
...
Триггерам можно присваивать произвольные имена и использовать эти имена в качестве [дополнительных условий](/automation/database/actions/#_1) для действий, а также вставлять их в [шаблоны](/automation/patterns/).
