...
Этот элемент шаблона позволяет получить значение простого XML-тега по его имени.
Функция ищет первое вхождение указанного элемента в XML-строке и возвращает его текстовое содержимое.
Вложенные теги и атрибуты не поддерживаются - возвращается только текст между открывающим и закрывающим тегами.

Пример: пусть устройство возвращает строку
```
<status>
  <power>on</power>
...
Для получения значения температуры можно использовать:
```
{{ xml.temperature }}
```
Все специальные символы (&lt;, &gt;, &amp; и т.п.) автоматически декодируются.
В случае некорректного XML-ввода или отсутвующего тега, функция вернёт пустое значение.
